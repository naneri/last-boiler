...
- **Frontend** — Vite with Tailwind CSS v4 (via `@tailwindcss/vite`) and Alpine.js (used for the navigation dropdown). All views rely on `@vite()`, so `npm run build` must run before the app renders. PostCSS config is intentionally absent (not needed in v4). `tailwindcss` v4 must be present at the top level to avoid conflict with `@tailwindcss/forms`.
